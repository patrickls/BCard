...
- Row Level Security (RLS) do Supabase **fica ativado, sem nenhuma policy**, em todas as tabelas de negócio (`users`, `verbs`, `preposition_translations`, `preposition_required_usage`, `preposition_to_for`, `preposition_in_on_at`). Isso bloqueia por padrão o acesso via PostgREST (roles `anon`/`authenticated` — inclusive alguém que extraia a `SUPABASE_ANON_KEY` pública do bundle do frontend). O backend não é afetado: o TypeORM conecta via `DATABASE_URL` usando o role `postgres` do Supabase, que tem `BYPASSRLS`, então a autorização continua sendo feita inteiramente na camada Service — ver seção 14 (2026-08-19, 2026-08-21). **Nunca** criar policies nessas tabelas nem desativar o RLS sem atualizar esta seção — qualquer uma das duas reabre o acesso direto e duplica a autorização entre Postgres e Service. **Toda nova tabela de negócio deve nascer com RLS ativado** (na própria migration ou logo após rodá-la) — não esperar o Supabase sinalizar "Table publicly accessible" de novo.
...
- **2026-08-21 — o fix de RLS de 2026-08-19 nunca tinha sido aplicado às tabelas de `prepositions`:** ao criar a 4ª tabela do módulo (`preposition_in_on_at`, Tipo D — in/on/at), uma checagem em `pg_class` mostrou que as 3 tabelas de prepositions já existentes (`preposition_translations`, `preposition_required_usage`, `preposition_to_for`) também estavam com `relrowsecurity = false` — o fix de 2026-08-19 cobriu só `users`/`verbs`, não o módulo inteiro. Mesmo sendo conteúdo didático (não dado de usuário), a mesma exposição via PostgREST com a anon key pública se aplicava. Corrigido com `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` (sem policy) nas 4 tabelas de prepositions, mesmo padrão de `users`/`verbs`. Lição: a regra da seção 7 vale para "toda tabela de negócio", não só as que motivaram o fix original — ao criar uma tabela nova, checar se as tabelas irmãs do mesmo módulo já seguem a regra antes de assumir que sim.