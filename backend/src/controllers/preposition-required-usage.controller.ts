...
import { PrepositionRequiredUsageService } from "../services/preposition-required-usage.service";

export class PrepositionRequiredUsageController {
  private service = new PrepositionRequiredUsageService();
  ...
  getRandom = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const limit = isNaN(count) || count < 1 ? 1 : Math.min(count, 4);
