...
import { PrepositionTranslationService } from "../services/preposition-translation.service";

export class PrepositionTranslationController {
  private service = new PrepositionTranslationService();
  ...
  getRandom = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const limit = isNaN(count) || count < 1 ? 1 : Math.min(count, 9);
